...
void setup() {
  ...
  bno.setExtCrystalUse(true);
    ...
    bno.getEvent(&event);
    orientation_start = event.orientation.x;
...
void loop() { 
       ...
       sensors_event_t event;
       bno.getEvent(&event);
       heading = event.orientation.x - orientation_start;
  ...
  void Vsync(int power, int direction){
  Serial.println("Geradeaus sync mit ");
  Serial.print(direction);

  if(heading < direction - 5){
    DR(power);
    delay(30);
    Stop();
  }
  else if(heading > direction + 5){
    DL(power);
    delay(30);
    Stop();
  }
